...
class ISOPENetwork(nn.Module):
    def __init__(self, train_stage):
        super(ISOPENetwork, self).__init__()
    ...
    def forward(self, depth, obj_id, camK,
        ...
        if self.train_stage == 'PoseNet_only':
            ...
            obj_mask = None
            PC, PC_Index = PC_sample(def_mask, depth, camK, gt_2D)
            if PC is None:
    ...
    def build_params(self, training_stage_freeze=None):
        ...
        if "face_recon" in training_stage_freeze:
            for param in zip(self.posenet.face_recon.parameters()):
                with torch.no_grad():
